...
const SAMPLE = Array.from(
  { length: 12 },
  () =>
    "Typing with intention is a small daily practice that builds speed, accuracy, and confidence over time. ",
).join("");
...
function getMetrics(text: string, elapsedSeconds: number): Omit<TestResult, "duration"> {
  let correctCharacters = 0;
  let mistakes = 0;

  for (let index = 0; index < text.length; index += 1) {
    if (text[index] === SAMPLE[index]) {
      correctCharacters += 1;
    } else {
      mistakes += 1;
    }
...
export default function TypingTestClient() {
  ...
  const liveMetrics = useMemo(() => getMetrics(typedText, elapsedSeconds), [typedText, elapsedSeconds]);
  ...
  function finishTest(text = typedText) {
    ...
    const testResult: TestResult = {
      ...getMetrics(text, Math.max(0.1, Math.min(duration, (Date.now() - startedAt) / 1000))),
      duration,
    };
  ...
  function handleTextChange(value: string) {
    ...
    const nextValue = value.slice(0, SAMPLE.length);
    if (startedAt === null && nextValue.length > 0) {
      const start = Date.now();
      setStartedAt(start);
      setNow(start);
    }
    setTypedText(nextValue);
    if (nextValue.length === SAMPLE.length && startedAt !== null) {
      ...
      <section className="mx-auto max-w-5xl px-6 pb-20 pt-10">
        ...
        {result ? (
          ...
          <div className="mt-8 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <div className="rounded-xl bg-slate-50 p-5 font-mono text-lg leading-8 tracking-wide text-slate-400" aria-label="Passage to type">
              {Array.from(SAMPLE).map((character, index) => {
            ...
            <label className="mt-5 block">
              ...
              <textarea
                ...
                className="w-full resize-none rounded-xl border border-slate-300 px-4 py-3 font-mono text-lg outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
              />
            </label>
            <p className="mt-3 text-center text-sm text-slate-500">The timer starts with your first character. Click the passage to refocus.</p>
          </div>
        )}
      </section>
    </main>
