        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-2" aria-label="Test duration">
          {DURATIONS.map((option) => (
            <button
              ...
              className={`rounded-lg px-4 py-2 text-sm font-semibold transition ${duration === option ? "bg-indigo-600 text-white" : "bg-white text-slate-600 ring-1 ring-slate-200 hover:bg-slate-100"
                }`}
            >
              {option / 60} min
            </button>
        ...
        <div className="mt-2 flex items-center justify-center gap-2" aria-label="Difficulty selector">
          {(["easy", "medium", "hard", "expert"] as Difficulty[]).map((level) => (
            <button
              ...
              className={`rounded px-3 py-1 text-xs font-medium ${difficulty === level ? "bg-indigo-700 text-white" : "bg-gray-200 text-gray-800"
                }`}
