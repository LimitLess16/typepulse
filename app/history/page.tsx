...
export default function HistoryPage() {
  ...
  if (loading || !user) {
    return <main className="flex min-h-screen items-center justify-center text-slate-600">Loading your history...</main>;
  }
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="mx-auto max-w-6xl px-6 py-12">
        ...
        <p className="mt-3 text-slate-600">{tests.length} saved {tests.length === 1 ? "test" : "tests"}, newest first.</p>
        {error && <p className="mt-6 rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</p>}
        {error ? (
          <div className="mt-10 rounded-2xl bg-white p-8 text-center ring-1 ring-slate-200">
            <p className="text-slate-600">Please try refreshing the page in a moment.</p>
          </div>
        ) : tests.length === 0 ? (
          <div className="mt-10 rounded-2xl bg-white p-8 text-center ring-1 ring-slate-200">
            <p className="text-slate-600">Complete your first test to start building a history.</p>
            ...
            <div className="divide-y divide-slate-100">
              {tests.map((test) => (
                <div key={test.id} className="grid gap-3 px-5 py-5 sm:grid-cols-5 sm:items-center">
                  <div><p className="text-2xl font-bold text-slate-900">{test.wpm.toFixed(1)} <span className="text-sm font-medium text-slate-500">WPM</span></p><p className="text-sm text-slate-500">{formatDate(getTypingTestTime(test))}</p></div>
...
function Stat({ label, value }: { label: string; value: string }) {
  return <div><p className="text-xs uppercase tracking-wide text-slate-400">{label}</p><p className="mt-1 font-semibold text-slate-700">{value}</p></div>;
}
